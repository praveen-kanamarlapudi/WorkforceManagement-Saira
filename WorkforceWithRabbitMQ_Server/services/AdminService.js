...
if(msg.requestQueue==="listGuard")
{
	...
	{
		var query = dbConn.query("select  firstName,lastName,userSSN as userSSN,userId ,phoneNumber as phno,addressLine1,addressLine2,city,state,zipCode,grd.startDate,grd.endDate,case when grd.backgroundStatus=1 then 'true' else 'false' end as status from userdetails as ud"+
									"inner join guard grd on userId = guardId where grd.isDeleted=0 ", function(err, rows){
			if(err)
			{
...
{		
	...
	//console.log("In RMQServer:Guard Service for Edit Guard");
	////console.log(guardDetails);	
	console.log(msg.backgroundStatus);
	var dbConn = myDatabase.getDatabaseConnection();
	if(dbConn === "empty")
	{
		//console.log("no DB connections available");
